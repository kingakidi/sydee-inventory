<?php 
            ...
            $revenueQuery = mysqli_query($conn, "SELECT * FROM `revenue` WHERE branch_id = $branchId AND (WEEK(date) = WEEK(now()))  ORDER BY date DESC");           
            if (!$revenueQuery) {
            ...
            $eQuery = mysqli_query($conn, "SELECT * FROM expenses WHERE branch_id=$branchId AND (WEEK(date) = WEEK(now()))  ORDER BY date DESC");
